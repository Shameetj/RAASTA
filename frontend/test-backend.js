...
async function runTests() {
  ...
  const routePayload = {
    start: {
      lat: 28.6328,
      lng: 77.2197,
      name: 'Metro Station Gate 2 (Elevator Access)'
    },
    destination: {
      lat: 28.6345,
      lng: 77.2185,
  ...
  try {
    ...
    if (res.ok) {
      const data = await res.json();
      console.log(`\x1b[32m✔ SUCCESS (${res.status} OK)\x1b[0m in ${latency}ms`);
      console.log('  Received Route Calculation:');
      console.log('\x1b[90m' + JSON.stringify(data, null, 2) + '\x1b[0m');
      passedCount++;
      ...
      console.log('  Frontend will continue using verified accessible fallback calculation.');
    } else {
      console.log(`\x1b[31m✖ FAILED (HTTP ${res.status} ${res.statusText})\x1b[0m`);
      const body = await res.text();
      console.log('  Response body:', body);
    ...
    console.log(`\x1b[31m✖ CONNECTION ERROR\x1b[0m: ${err.message}`);
    console.log('  Frontend will seamlessly fallback to verified client-side step-free routing.');
  }
